...
from typing import Any, List, Optional, Sequence, Set, Text, Tuple, Type, Union, cast
...
def is_typing_list(data_type: Any) -> bool:
    ...
    return data_type is List or (
        ...
        and data_type.__origin__ in  # type: ignore
...
def coerce_to_type(
    ...
    *,
    ...
    if is_list:
        output = convert_as_yaml(value, f"Cannot convert value '{value}' to a list of '{valid_types}'")
        ...
        if not all(isinstance(x, valid_types) for x in output):
            raise ValueError(f"Cannot convert value '{value}' to a list of '{valid_types}', got {output}")
        return output
    ...
    *,
    ...
    if isinstance(value, list):
        if data_type is not None:
            ...
            if not is_list:
                raise TypeError(f"Cannot convert input '{value}' of type '{type(value)}' to" f" '{data_type}'")
...
def normalize_typed_substitution(value: SomeValueType, data_type: Optional[AllowedTypesType]) -> NormalizedValueType:
    ...
    err_msg = "Got a list of '{}'" f", expected a list of '{data_type}'. value='{value}'"
    ...
    if types_in_list.issubset({str, Substitution}):
        ...
        return cast(
            ...
            [
                x
                if isinstance(x, str)
                else normalize_to_list_of_substitutions(  # Don't convert strings to TextSubstitution
                    cast(SomeSubstitutionsType, x)
                )
                for x in value
            ],
        )
    if types_in_list.issubset({bool, Substitution}):
        ...
        return cast(
            ...
            [
                normalize_to_list_of_substitutions(cast(SomeSubstitutionsType, x)) if is_substitution(x) else x
                for x in value
            ],
        )
    ...
    # Invalid input
    raise TypeError("Input value is not an acceptable 'SomeValueType'." f"Got value='{value}'")
...
def perform_typed_substitution(
    ...
    if isinstance(value, ScalarTypesTuple):
        ...
        return coerce_to_type(perform_substitutions(context, cast(List[Substitution], value)), data_type)
        ...
        return cast(ListValueType, output)
    # Invalid input
    raise TypeError("Input value is not an acceptable 'NormalizedValueType'." f"Got value='{value}'")
