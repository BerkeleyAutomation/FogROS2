...
def format_action(action: Action) -> List[Text]:
    ...
    if is_a(action, LogInfo):
        ...
        msg = "ExecuteProcess(cmd=[{}], cwd={}, env={}, shell={})".format(
            ...
            typed_action.cwd if typed_action.cwd is None else "'{}'".format(format_substitutions(typed_action.cwd)),
            typed_action.env
            if typed_action.env is None
            else "{"
            + ", ".join(
                ["{}: {}".format(format_substitutions(k), format_substitutions(v)) for k, v in typed_action.env]
            )
            + "}",
            typed_action.shell,
        )
