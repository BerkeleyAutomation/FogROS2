...
class Command(Substitution):
    ...
    def __init__(self, command: SomeSubstitutionsType, *, on_stderr: SomeSubstitutionsType = "fail") -> None:
        """
        ...
        from ..utilities import (
            normalize_to_list_of_substitutions,  # import here to avoid loop
        )
    ...
    def perform(self, context: LaunchContext) -> Text:
        ...
        if on_stderr not in ("fail", "ignore", "warn", "capture"):
            raise SubstitutionFailure("expected 'on_stderr' to be one of: 'fail', 'ignore', 'warn' or 'capture'")
        stderr = subprocess.PIPE
        if on_stderr == "capture":
            stderr = subprocess.STDOUT

        try:
            result = subprocess.run(command, stdout=subprocess.PIPE, stderr=stderr, universal_newlines=True)
