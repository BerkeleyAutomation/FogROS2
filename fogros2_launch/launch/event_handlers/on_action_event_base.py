...
from typing import TYPE_CHECKING, Callable, Optional, Text, Type, Union
...
class OnActionEventBase(BaseEventHandler):
    ...
    def __init__(
        ...
        *,
        ...
        on_event: Union[SomeActionsType, Callable[[Event, LaunchContext], Optional[SomeActionsType]]],
        ...
        **kwargs,
        ...
        if not isinstance(action_matcher, (target_action_cls, type(None))) and not callable(action_matcher):
            raise TypeError(f"action_matcher must be an '{target_action_cls.__name__}' instance or a callable")
    ...
    def matcher_description(self) -> Text:
        ...
        return f"event == {self.__target_event_cls.__name__} and" f" {self.__target_action_cls.__name__}(event.action)"
