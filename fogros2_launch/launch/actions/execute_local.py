...
from typing import Callable, Dict, List, Optional, Text, Union, cast

import launch.logging
from osrf_pycommon.process_utils import AsyncSubprocessProtocol, async_execute_process
...
from ..events import Shutdown, matches_action
...
from ..substitutions import LaunchConfiguration, PythonExpression
...
class ExecuteLocal(Action):
    ...
    def __init__(
        ...
        *,
        ...
        log_cmd: bool = False,
        on_exit: Optional[
            Union[SomeActionsType, Callable[[ProcessExited, LaunchContext], Optional[SomeActionsType]]]
        ] = None,
        respawn: bool = False,
    ...
    def _shutdown_process(self, context, *, send_sigint):
        ...
        if self.process_details is None or self._subprocess_transport is None:
            ...
            context.register_event_handler(
                OnProcessStart(
                    on_start=lambda event, context: self._shutdown_process(context, send_sigint=send_sigint)
                )
            )
            return None
    ...
    def __on_signal_process_event(self, context: LaunchContext) -> Optional[LaunchDescription]:
        ...
        if platform.system() == "Windows" and typed_event.signal_name == "SIGINT":
            ...
            typed_event = SignalProcess(signal_number=signal.SIGTERM, process_matcher=lambda process: True)
        self.__logger.info(
            "sending signal '{}' to process[{}]".format(typed_event.signal_name, self.process_details["name"])
        )
        try:
    ...
    def __on_process_stdin(self, event: ProcessIO) -> Optional[SomeActionsType]:
        self.__logger.warning(
    ...
    def __on_process_output(
        ...
        if buffer.closed:
            ...
            # buffering.
            buffer.info(self.__output_format.format(line=to_write, this=self))
        else:
    ...
    def __on_process_output_cached(self, event: ProcessIO, buffer, logger) -> Optional[SomeActionsType]:
        to_write = event.text.decode(errors="replace")
    ...
    def __get_shutdown_timer_actions(self) -> List[Action]:
        ...
        sigkill_timeout = [
            PythonExpression(("float(", *self.__sigterm_timeout, ") + float(", *self.__sigkill_timeout, ")"))
        ]
        ...
        self.__sigterm_timer = TimerAction(
            ...
            actions=[
                OpaqueFunction(
                    function=printer, args=(base_msg.format("{}", "{}", "SIGINT", "SIGTERM"), sigterm_timeout)
                ),
                EmitEvent(event=SignalProcess(signal_number=signal.SIGTERM, process_matcher=matches_action(self))),
            ],
        ...
        self.__sigkill_timer = TimerAction(
            ...
            actions=[
                OpaqueFunction(
                    function=printer, args=(base_msg.format("{}", "{}", "SIGTERM", "SIGKILL"), sigkill_timeout)
                ),
                EmitEvent(event=SignalProcess(signal_number="SIGKILL", process_matcher=matches_action(self))),
            ],
            cancel_on_shutdown=False,
        )
        return [
    ...
    async def __execute_process(self, context: LaunchContext) -> None:
        ...
        try:
            transport, self._subprocess_protocol = await async_execute_process(
                lambda **kwargs: self.__ProcessProtocol(self, context, process_event_args, **kwargs),
                cmd=cmd,
                cwd=cwd,
                env=env,
                shell=self.__shell,
        ...
        if returncode == 0:
            ...
            self.__logger.error(
                "process has died [pid {}, exit code {}, cmd '{}'].".format(pid, returncode, " ".join(cmd))
            )
    ...
    def execute(self, context: LaunchContext) -> Optional[List[LaunchDescriptionEntity]]:
        ...
        if self.__executed:
            raise RuntimeError(f"ExecuteLocal action '{name}': executed more than once: {self.describe()}")
        ...
        event_handlers = [
            ...
            OnProcessIO(
                ...
                on_stdout=lambda event: on_output_method(event, self.__stdout_buffer, self.__stdout_logger),
                on_stderr=lambda event: on_output_method(event, self.__stderr_buffer, self.__stderr_logger),
            ),
            OnShutdown(
        ...
        try:
            ...
            self.__stdout_logger, self.__stderr_logger = launch.logging.get_output_loggers(name, self.__output)
            context.asyncio_loop.create_task(self.__execute_process(context))
        except Exception:
            for event_handler in event_handlers:
    ...
    def get_stdout(self):
        ...
        if not self.__cached_output:
            raise RuntimeError(
                "cached output must be true to be able to get stdout," f" proc '{self.__process_description.name}'"
            )
    ...
    def get_stderr(self):
        ...
        if not self.__cached_output:
            raise RuntimeError(
                "cached output must be true to be able to get stderr, proc" f" '{self.__process_description.name}'"
            )
