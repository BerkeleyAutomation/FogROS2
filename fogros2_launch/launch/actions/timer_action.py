...
from typing import Iterable, List, Optional, Text, Tuple, Union, cast
...
from ..utilities import create_future, ensure_argument_type, is_a_subclass, type_utils
...
class TimerAction(Action):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(period, str):
            period = float(period)
            warnings.warn(
                "The parameter 'period' must be a float or substitution," "passing a string literal was deprecated",
                stacklevel=2,
            )
        ...
        self.__cancel_on_shutdown = type_utils.normalize_typed_substitution(cancel_on_shutdown, bool)
    ...
    def parse(
        ...
        kwargs["period"] = parser.parse_if_substitutions(entity.get_attr("period", data_type=float, can_be_str=True))
        kwargs["actions"] = [parser.parse_action(child) for child in entity.children]
        cancel_on_shutdown = entity.get_attr("cancel_on_shutdown", optional=True, data_type=bool, can_be_str=True)
    ...
    def describe_conditional_sub_entities(
        self,
    ) -> List[Tuple[Text, Iterable["LaunchDescriptionEntity"],]]:
            ...
            context.register_event_handler(
                EventHandler(
                    ...
                    entities=OpaqueFunction(
                        function=lambda context: (cast(TimerEvent, context.locals.event).timer_action.handle(context))
                    ),
                )
            )
