...
@pytest.mark.parametrize("test_input,expected", [(None, [True, False]), ({"TEST_NEW_ENV": "2"}, [False, True])])
def test_execute_process_with_env(test_input, expected):
    ...
    executable = ExecuteProcess(
        cmd=[sys.executable, "TEST_PROCESS_WITH_ENV"], output="screen", env=test_input, additional_env=additional_env
    )
...
def test_execute_process_with_on_exit_behavior():
    ...
    executable_with_on_exit_callback = ExecuteProcess(
        cmd=[sys.executable, "-c", "print('callback')"], output="screen", on_exit=on_exit_callback
    )
    ...
    executable_with_on_exit_action = ExecuteProcess(
        cmd=[sys.executable, "-c", "print('action')"], output="screen", on_exit=[on_exit_action]
    )
    assert executable_with_on_exit_action.get_sub_entities() == [on_exit_action]

    ld = LaunchDescription([executable_with_on_exit_callback, executable_with_on_exit_action])
    ls = LaunchService()
    ls.include_launch_description(ld)
    assert 0 == ls.run()
            ...
            [
                ...
                RegisterEventHandler(
                    event_handler=OnProcessStart(
                        ...
                        on_start=[EmitEvent(event=ShutdownEvent(reason="none", due_to_sigint=True))],
                    )
                ),
            ]
        )
    ...
    def generate_launch_description():
        return LaunchDescription(
            [
                ...
                TimerAction(period=shutdown_time, actions=[Shutdown(reason="Timer expired")]),
            ]
        )
...
def test_execute_process_prefix_filter_match():
    ...
    SetLaunchConfiguration("launch-prefix-filter", f"{os.path.basename(sys.executable)}").visit(lc)
    assert len(lc.launch_configurations) == 2

    test_process = ExecuteProcess(cmd=[sys.executable, "-c", "print('action')"], output="screen")
...
def test_execute_process_prefix_filter_override_in_launch_file():
    ...
    test_process = ExecuteProcess(prefix="echo", cmd=[sys.executable, "-c", "print('action')"], output="screen")

    test_process.execute(lc)
    assert "echo" in test_process.process_details["cmd"] and "time" not in test_process.process_details["cmd"]
