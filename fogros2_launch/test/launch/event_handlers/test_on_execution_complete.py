...
def test_bad_construction():
    ...
    with pytest.raises(TypeError):
        OnExecutionComplete(target_action="not-an-action", on_completion=lambda *args: None)

    with pytest.raises(TypeError):
        OnExecutionComplete(
            target_action=LogInfo(msg="some message"), on_completion="not-a-callable-nor-an-action-iterable"
        )
...
def test_single_action_is_matched():
    ...
    event_handler = OnExecutionComplete(target_action=an_action, on_completion=lambda *args: None)
