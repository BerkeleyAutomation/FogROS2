...
from launch.frontend.parse_substitution import (
...
def test_quoted_nested_substitution():
    subst = parse_substitution("go_to_$(env WHERE asd)_of_$(env " "'something $(test 10)')")
    assert len(subst) == 4
...
def test_double_quoted_nested_substitution():
    subst = parse_substitution(r'$(env "asd_bsd_qsd_$(test \"asd_bds\")" "$(env DEFAULT)_qsd")')
...
def test_combining_quotes_nested_substitution():
    subst = parse_substitution("$(env \"asd_bsd_qsd_$(test 'asd_bds')\" '$(env DEFAULT)_qsd')")
    context = LaunchContext()
    assert len(subst) == 1
    assert len(subst[0].name) == 2
    assert subst[0].name[0].perform(context) == "asd_bsd_qsd_"
