...
def test_group_action_execute():
    ...
    assert len(lc1.launch_configurations) == 0
    result = GroupAction([], forwarding=True, launch_configurations={"foo": "FOO", "bar": "BAR"}).visit(lc1)
    ...
    assert len(lc1.launch_configurations) == 0
    result = GroupAction([], forwarding=False, launch_configurations={"foo": "FOO", "bar": "BAR"}).visit(lc1)
    ...
    assert len(lc1.launch_configurations) == 0
    result = GroupAction([Action()], forwarding=True, launch_configurations={"foo": "FOO"}).visit(lc1)
    ...
    result = GroupAction(
        [Action()], forwarding=False, launch_configurations={"bar": LaunchConfiguration("bar"), "baz": "BAZ"}
    ).visit(lc1)
    ...
    lc1.launch_configurations["foo"] = "FOO"
    lc1.launch_configurations["bar"] = "BAR"
    result = GroupAction([Action()], forwarding=True, launch_configurations={"foo": "OOF"}).visit(lc1)
