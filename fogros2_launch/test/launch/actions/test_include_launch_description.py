...
def test_include_launch_description_constructors():
    ...
    IncludeLaunchDescription(LaunchDescriptionSource(LaunchDescription()), launch_arguments={"foo": "FOO"}.items())
...
def test_include_launch_description_launch_arguments():
    ...
    action1 = IncludeLaunchDescription(LaunchDescriptionSource(ld1))
    ld2 = LaunchDescription(
        [DeclareLaunchArgument("foo2"), ResetLaunchConfigurations(), SetLaunchConfiguration("foo", "asd"), action1]
    )
    action2 = IncludeLaunchDescription(
        LaunchDescriptionSource(ld2),
        launch_arguments={"foo2": "FOO2"}.items(),
...
def test_include_python():
    ...
    simple_launch_file_path = os.path.join(this_dir, "..", "launch_description_source", "simple.launch.py")
