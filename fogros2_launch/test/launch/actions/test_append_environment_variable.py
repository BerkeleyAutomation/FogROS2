...
def test_append_environment_variable_execute():
    ...
    assert os.environ.get("NONEXISTENT_KEY") == "some value" + os.pathsep + "value" + os.pathsep + "another value"

    # Can use an optional separator
    AppendEnvironmentVariable("NONEXISTENT_KEY", "other value", separator="|").visit(lc1)
    assert (
        os.environ.get("NONEXISTENT_KEY")
    ...
    AppendEnvironmentVariable(
        "ANOTHER_NONEXISTENT_KEY", EnvironmentVariable("NONEXISTENT_KEY"), prepend=TextSubstitution(text="false")
    ).visit(lc1)
    assert (
        os.environ.get("ANOTHER_NONEXISTENT_KEY")
        == "some value" + os.pathsep + "value" + os.pathsep + "another value" + "|" + "other value"
    )
    ...
    AppendEnvironmentVariable(
        ...
        prepend=TextSubstitution(text="yes"),
    ).visit(lc1)
