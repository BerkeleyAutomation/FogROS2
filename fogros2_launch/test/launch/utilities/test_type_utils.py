...
def test_is_instance_of_valid_type(is_instance_of_valid_type_impl):
    ...
    assert is_instance_of_valid_type_impl(1.0)
    ...
    assert is_instance_of_valid_type_impl([1.0, 2.0])
    ...
    assert is_instance_of_valid_type_impl(1.0, can_be_str=True)
    ...
    assert is_instance_of_valid_type_impl([1, "2"], can_be_str=True)
    assert is_instance_of_valid_type_impl([1.0, "2."], can_be_str=True)
    ...
    assert not is_instance_of_valid_type_impl([1, "2", 1.0], can_be_str=True)
...
def test_is_instance_of():
    ...
    assert is_instance_of(["1", 2], List[int], can_be_str=True)
    assert not is_instance_of(["1", 2.0], List[int], can_be_str=True)
    assert not is_instance_of([1, 2.0], List[int], can_be_str=True)
...
@pytest.mark.parametrize(
    ...
    (
        coerce_to_type,
        lambda value, data_type=None, can_be_str=False: get_typed_value(value, data_type, can_be_str=can_be_str),
    ),
    ids=[
...
def test_coercions_using_yaml_rules(coerce_to_type_impl):
    ...
    assert coerce_to_type_impl("1000.0") == 1000.0
    assert coerce_to_type_impl("0.2") == 0.2
    ...
    assert coerce_to_type_impl("[.2, .1, .1]") == [0.2, 0.1, 0.1]
...
def test_coercions_given_specific_type(coerce_to_type_impl):
    ...
    assert coerce_to_type_impl("1.", data_type=float) == 1.0
    assert coerce_to_type_impl("1000.0", data_type=float) == 1000.0
    assert coerce_to_type_impl("0.2", data_type=float) == 0.2
    ...
    assert coerce_to_type_impl("[.2, .1, .1]", data_type=List[float]) == [0.2, 0.1, 0.1]
    ...
    assert coerce_to_type_impl("asd", data_type=float, can_be_str=True) == "asd"
    assert coerce_to_type_impl("[asd, 2.0]", data_type=List[float], can_be_str=True) == ["asd", 2.0]
...
def test_coercing_list_using_yaml_rules(coerce_list_impl):
    ...
    assert coerce_list_impl(["1.", "1000."]) == [1.0, 1000.0]
    ...
    assert coerce_list_impl(["asd", "1000."], data_type=None, can_be_str=True) == ["asd", 1000.0]
    assert coerce_list_impl(["asd", "off", "True"], data_type=None, can_be_str=True) == ["asd", False, True]
...
@pytest.mark.parametrize(
    ...
    (
        coerce_list,
        lambda value, data_type=None, can_be_str=False: get_typed_value(value, List[data_type], can_be_str=can_be_str),
    ),
    ids=[
        "testing coerce_list implementation",
        "testing get_typed_value implementation",
...
def test_is_normalized_substitution():
    ...
    assert is_normalized_substitution([TextSubstitution(text="asd"), TextSubstitution(text="bsd")])
...
def test_perform_typed_substitution():
    ...
    assert perform_typed_substitution(lc, [TextSubstitution(text="[1, 2, 3]")], List[int]) == [1, 2, 3]
    assert perform_typed_substitution(lc, [TextSubstitution(text="[1, 2, 3]")], None) == [1, 2, 3]
    ...
    with pytest.raises(TypeError):
        perform_typed_substitution(lc, 1.0, int)
    with pytest.raises(ValueError):
    ...
    with pytest.raises(ValueError):
        perform_typed_substitution(lc, [[TextSubstitution(text="1.")], [TextSubstitution(text="1")]], data_type=None)
