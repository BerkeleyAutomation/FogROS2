...
from launch.actions import ExecuteLocal, OpaqueFunction, Shutdown, TimerAction
...
def test_execute_process_with_env(test_input, expected):
    ...
    executable = ExecuteLocal(
        process_description=Executable(
            cmd=[sys.executable, "TEST_PROCESS_WITH_ENV"], env=test_input, additional_env=additional_env
        ),
        output="screen",
    )
