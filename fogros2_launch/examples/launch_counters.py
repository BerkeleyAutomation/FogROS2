...
def main(argv=sys.argv[1:]):
    ...
    ld = LaunchDescription(
        [
            ...
            launch.actions.LogInfo(
                msg=("Is that you, ", launch.substitutions.EnvironmentVariable(name=user_env_var), "?")
            ),
        ]
    )
    ...
    def counter_output_handler(event):
        ...
        if target_str in event.text.decode():
            return launch.actions.EmitEvent(
                event=launch.events.Shutdown(reason="saw '{}' from '{}'".format(target_str, event.process_name))
            )
    ...
    ld.add_action(launch.actions.ExecuteProcess(cmd=[sys.executable, "-u", "./counter.py", "--ignore-sigint"]))
    ld.add_action(
